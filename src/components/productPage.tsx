...
export const ProductPage:React.FC = (props:any) => {
    ...
    const [product]:any = _.filter<Array<Iproducts>>(products, (o:Iproducts) => {
        console.log(o)
    ...
    const trolley = () => {
        dispatch(moveToTrolley({id:product.id, name:product.name, price:product.price, state:product.state, imgAdress:product.imgAdress, seller:product.seller, type: product.type, desc: product.desc}))
            ...
            {products.length > 0 ?
                ...
                <main className="productPage">
                    <section className="mainSection">
                        <figure><img src={products[props.match.params.id].imgAdress} alt="img"/></figure>
                        <article className="info">
                            ...
                            <a href="#description"><span>zobacz pełny opis</span></a>
                        </article>
                    </section>
                    <section className="payment">
                        <h3>zamów w ciągu <span className="rounded">{Math.floor((Math.random()*14)+17)} dni</span></h3>
                        <div className="rpices">
                            <span className="grey">dostępnych sztuk: {Math.floor((Math.random()*144)+3)}</span>
                            <h4>{product.price}</h4>
                        </div>
                        <div className="cont">
